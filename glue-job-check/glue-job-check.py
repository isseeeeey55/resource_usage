...
f_date = jst_now - timedelta(days=1)
t_date = jst_now - timedelta(days=0)
from_date = f_date.strftime('%Y-%m-%d 00:00:00')
to_date = t_date.strftime('%Y-%m-%d 00:00:00')
...
file_name = 'Glue-UsageReport-' + file_date + '.csv'
...
def main(event, context):
    ...
    for id in account_list:
        ...
        for job in jobs:
            name = job['Name']
            job_run_list = session.client('glue').get_job_runs(
                JobName = name,
                MaxResults = 200
            )
            ...
            history = job_run_list.get('JobRuns')
            for key in history:
                result = []
